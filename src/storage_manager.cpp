                              ...
                              const std::vector<std::string> &outgoing_links)
{
    ...
    // Store in thread-local buffer — UNION edges (not overwrite)
    auto& existing = buffer.local_graph[domain];
    for (const auto& d : outgoing_domains) {
        // Only add if not already present (dedup per-thread)
        if (std::find(existing.begin(), existing.end(), d) == existing.end()) {
...
void StorageManager::merge_all_buffers()
{
    ...
    {
        // Merge graph — UNION edges across all threads (not last-write-wins)
        for (const auto &[domain, links] : buffer.local_graph)
        {
            auto& dest = link_graph[domain];
            for (const auto& lnk : links) {
                if (std::find(dest.begin(), dest.end(), lnk) == dest.end()) {
                    dest.push_back(lnk);
                }
            }
        }
